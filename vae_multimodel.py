...
def gaussian_MLP_encoder(x, n_hidden, n_output, keep_prob):
    with tf.variable_scope("gaussian_MLP_encoder"):
        ...
        with tf.name_scope("w0"):
            w0 = tf.get_variable('w0', [x.get_shape()[1], x.get_shape()[2], n_hidden], initializer=w_init)
...
#         tf.summary.image('generated', tf.reshape(y, [-1, 28, 28, 1]), 10)
...
#     y = tf.clip_by_value(y, 1e-6, 1- 1e-6)
    ...
    KL_divergence = 0.5 * tf.reduce_sum(tf.square(mu) + tf.square(sigma) - tf.square(sigma)) - 1, 1
