...
if __name__ == "__main__":
    ...
    def_dict = {}
#    for file_path in tqdm.tqdm(glob.glob("share_event_nofilter/def_*")):
...
#        
#        event_obj = open_dump("share_event_nofilter/event_"+file_path.split("def_")[-1])
#        for k, v in event_obj.items():
#            dump_name = tmp_def_dict[k]
#            date = file_path.split("_")[-1]
#            with open("share_event_nofilter_tmp/{0}_{1}.dump".format(dump_name, date), "wb") as f:
#                pickle.dump(v, f)

    print("make one day long dumps...")

    for i in tqdm.tqdm(set(["_".join(i.split("/")[-1].split("_")[:2]) for i in glob.glob("share_event_nofilter_tmp/*")])):
        time_series = []
        for j in glob.glob("share_event_nofilter_tmp/"+i+"*"):
            obj = open_dump(j)
            time_series.extend(obj)
        with open("nofilter_oneday/"+i+".dump", "wb") as f:
